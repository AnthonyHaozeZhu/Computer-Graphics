...
struct KDTree {
    ...
    bool IsHit(const SimplePathTracer::Ray& r, KDNode* n) {
    ...
    std::list<KDNode*> find_Node(const SimplePathTracer::Ray& r){
        std::list<KDNode*> result;
        find_Node(r, root, result);
        return result;
    }

    void find_Node(const SimplePathTracer::Ray& r, KDNode* n, std::list<KDNode*>& result) {
        if (IsHit(r, n)) {
            if (n->is_leaf) {
                result.push_back(n);
            }
