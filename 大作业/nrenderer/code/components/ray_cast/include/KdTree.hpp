...
struct KDTree {
    ...
    bool IsHit(const RayCast::Ray& r, KDNode* n) {
    ...
    std::list<KDNode*> find_Node(const RayCast::Ray& r){
        std::list<KDNode*> result;
        find_Node(r, root, result);
        return result;
    }

    void find_Node(const RayCast::Ray& r, KDNode* n, std::list<KDNode*>& result) {
        if (IsHit(r, n)) {
            if (n->is_leaf) {
                result.push_back(n);
            }
